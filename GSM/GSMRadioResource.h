...
struct RachInfo {
	unsigned mRA;
	const kneedeepbts::gsm::GsmTime mWhen;
	RadData mRadData;
	unsigned mTN;
    ...
    kneedeepbts::gsm::GsmTime mReadyTime;	// When the SACCH is known to have started transmiting correctly.

	int initialTA() const { return mRadData.mTimingError; }
	float RSSI() const { return mRadData.mRSSI; }

	// Gotta love this language.
	RachInfo(unsigned wRA, const kneedeepbts::gsm::GsmTime &wWhen, RadData wRD, unsigned wTN=0)
